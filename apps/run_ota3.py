...
command2 = '/gr-mapper/apps/prbs_test_ota2.py -t '+param1+' -r '+param2+' > '+file
    ...
    with open(file_path, 'r') as file:
        for line in file: # we only need last line of pft
            if line.startswith('NBits'):
                label1, _nbit, label2, _nerror, label3, _ber, label4, _wtime = line.strip().split()
                nbit = int(_nbit)
                nerror = int(_nerror)
                ber = float(_ber)
                wtime=float(_wtime)
    return nbit, nerror, ber, wtime

# Packet error rate
file_path = file #per_file  # file path with recorded PER measurements
nbit, nerror, ber, wtime = parse_file(file_path)
print(f'BER: {ber}', 'NBits', nbit, 'NErrors', nerror, 'Time', wtime)
